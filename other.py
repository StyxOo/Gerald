...
def exponential(x, amp, yshift):
    ...
    return amp * math.e ** (x) + yshift  # Funktion


# Startwerte Paramater der Exponentialfunktion für den Fit festlegen


exponential_amp_guess = float((d[np.argwhere(x == max(x))] - d[np.argwhere(x == min(x))]) / (exp(max(x)) - exp(min(x))))

exponential_yshift_guess = float(d[np.argwhere(x == max(x))] - exponential_amp_guess * exp(max(x)))
...
g1_cen_guess = float(x[
                         g1_max_index])  # Energie an der Indexstelle des Maximums von d im Bereich vom Index der linken Gauss-Grenze bis zum Index der rechten Gauss-Grenze der Energie

g1_amp_guess = float(d[g1_max_index] - float(exponential(x[g1_max_index], exponential_amp_guess,
                                                 exponential_yshift_guess)))  # Maximums von d im Bereich vom Index der linken Gauss-Grenze bis zum Index der rechten Gauss-Grenze der Energie, abzüglich vom Funktionswert der Exponentialfunktion an der Stelle des Energiemaximums der Gaussfunktion

g1_wid_guess = 0.035  # HWHM oder Standardabweichung
...
g2_cen_guess = float(x[
                         g2_max_index])  # Energie an der Indexstelle des Maximums von d im Bereich vom Index der linken Gauss-Grenze bis zum Index der rechten Gauss-Grenze der Energie

g2_amp_guess = float(d[g2_max_index] - float(exponential(x[g2_max_index], exponential_amp_guess,
                                                 exponential_yshift_guess)))  # Maximums von d im Bereich vom Index der linken Gauss-Grenze bis zum Index der rechten Gauss-Grenze der Energie, abzüglich vom Funktionswert der Exponentialfunktion an der Stelle des Energiemaximums der Gaussfunktion

g2_wid_guess = 0.03  # HWHM oder Standardabweichung

# Fitfunktion der Exponentialfunktion erstellen


linmodel = Model(exponential, prefix='exp_')  # erster Slot: Funktion, prefix = Benennung der Funktion, relevant für Komponentendarstellung im Diagramm

pars = linmodel.make_params()

pars['exp_amp'].set(value=exponential_amp_guess)

pars['exp_yshift'].set(value=exponential_yshift_guess)
...
axes[1].plot(x, comps['exp_'], 'g--', label='Exponentielle Komponente')
...
results_file = 'Daten\Fit-Vorbereitung\Fitergebnisse'

plt.savefig(f"./{os.path.join(results_file, asc_name)[:-4]}_fit_plot.pdf")
...
outfile = open(f"./{os.path.join(results_file, asc_name)[:-4]}_fit_statistic.txt", "w+")
